...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await register(username, email, password, favoriteTeam);
    if (res.success) {
      navigate('/');
    } else {
      setError(res.error);
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-10 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white tracking-wide">Create Scout Account</h2>
          <p className="text-sm text-slate-400 mt-1">Join the AuctionIQ Network</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Favorite IPL Franchise</label>
            <select
              value={favoriteTeam}
              onChange={(e) => setFavoriteTeam(e.target.value)}
              className="w-full px-4 py-3 rounded-xl border border-slate-800 bg-slate-950 focus:outline-none focus:border-primary-500 text-white transition-all text-sm"
            >
              {teams.map(t => (
        ...
        <div className="text-center mt-8 pt-6 border-t border-slate-800">
          <p className="text-sm text-slate-400">
            ...
            <Link to="/login" className="text-primary-400 hover:underline font-semibold transition-all">
              Sign in here
            </Link>
          </p>
        </div>
